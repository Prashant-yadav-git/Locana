  ...
  const processImage = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // If the API returns product data, add to stock
        if (result.products && Array.isArray(result.products)) {
          result.products.forEach((product: any) => {
            addStockItem(product.name, product.quantity || 1)
            updateLocalStorage(product.name, product.quantity || 1)
          })
        } else {
          // Fallback: try to extract product from message
          const productMatch = result.message.match(/added (\w+)/i)
          if (productMatch) {
